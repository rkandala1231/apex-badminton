...
import { SubTabs, EmptyState } from '../matchcenter/shared';
  ...
  return (
    <div>
      ...
      <p className="text-[0.95rem] mb-5 max-w-[60ch] text-text-secondary">
        Seed and manage the bracket for each event, including Mixed Doubles.
      </p>
      ...
      {matches.length === 0 ? (
        <EmptyState
          className="mt-5"
          text={`The ${evLabel} bracket hasn't been seeded yet. Seeding tools will appear here once registration closes.`}
        />
      ) : (
        <div className="mt-5">{/* bracket editor renders here */}</div>
      )}
    </div>
  );
}
